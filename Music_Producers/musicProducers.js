...
http.open('get', 'musicProducers.json', true);
http.send();
http.onload = function(){
    if(this.readyState == 4 && this.status == 200){
        let MusicProducerss = JSON.parse(this.responseText);
        let output = "";
        for(let item of MusicProducerss){
            output += `
            ...
            <p class="cart">Contact Music Producer <i class="bx bx-cart-alt"></i></p>
            </div>
			`;
		}
		document.querySelector(".musicProducerss").innerHTML = output;
	}
}