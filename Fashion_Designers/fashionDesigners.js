...
http.open('get', 'fashionDesigners.json', true);
http.send();
http.onload = function(){
    if(this.readyState == 4 && this.status == 200){
        let FashionDesignerss = JSON.parse(this.responseText);
        let output = "";
        for(let item of FashionDesignerss){
            output += `
            ...
            <p class="cart">Contact Fashion Designer <i class="bx bx-cart-alt"></i></p>
            </div>
			`;
		}
		document.querySelector(".fashionDesignerss").innerHTML = output;
	}
}